server <- function(input, output, session) {
  ...
  observeEvent(input$Run,{
    ...
    values$PDF_CMI <-pdf_text(input$PDF_file1$datapath) %>% 
      str_split("\n")
    CMI_fun_output<- CMI_fun(values$PDF_CMI)
    CMI_Df <- CMI_fun_output$CMI_Df
    CMI_Date <- CMI_fun_output$CMI_Date %>% as.character()
    
    if(length(input$PDF_file2)>0){
      values$PDF_ABC <-pdf_text(input$PDF_file2$datapath)%>% 
        str_split("\n")
      ABC_fun_output<- ABC_fun(values$PDF_ABC)
      ABC_Df <- ABC_fun_output$ABC_Df
      ABC_Date <- ABC_fun_output$ABC_Date %>% as.character()
    }
    ## -----------------------------
    if(length(input$PDF_file2)>0){
    ...
    if(nrow(CMI_Df)>0 & !(input$Update_HD)){
    ...
    if(input$Update_HD){
      ...
      #### Deleting rows with today's date------------------
      ...
      colnames(DF_CMI_stock)<- c( "Reference name", as.character(CMI_Date) )
      ...
      colnames(CMI_Missing_from_PDf) = c("`Item name ","Reference name", as.character(CMI_Date))
      ...
      CMI_Missing_from_PDf<-CMI_Missing_from_PDf[order(CMI_Missing_from_PDf[,1]),]
      ...
      ####  if an item name from the sheet doesn't match with reference name Pdf item's name 
      ...
      CMI_NewItems_in_PDf<-CMI_NewItems_in_PDf[order(CMI_NewItems_in_PDf[,1]),]%>% as.data.frame()
      ...
      if(length(input$PDF_file2)>0){
        ...
        colnames(DF_ABC_stock)<- c( "Reference name", as.character(ABC_Date) )
        ...
        colnames(ABC_Missing_from_PDf) = c("`Item name ","Reference name", as.character(ABC_Date))
        ...
        ABC_Missing_from_PDf<-ABC_Missing_from_PDf[order(ABC_Missing_from_PDf[,1]),]
